...
const uvc_controls_t uvc_controls = {
    ...
    .led = {
        .unit = UVC_PROCESSING_UNIT_ID, //UVC_INPUT_TERMINAL_ID,
        .selector = 0x07, //
        .size = 1,
        .name = @"led",
    }
...
//    int intval = (enabled ? 0x01 : 0x00);
    ...
    printf("setLED = %i \n",enabled);
    return [self setData:intval 
              withLength:uvc_controls.led.size 
             forSelector:uvc_controls.led.selector 
                      at:uvc_controls.led.unit];
...
- (BOOL)getLED {
    int intval = [self getDataFor:UVC_GET_CUR 
                       withLength:uvc_controls.led.size 
                     fromSelector:uvc_controls.led.selector 
                               at:uvc_controls.led.unit];
